...
/*   Updated: 2017/02/05 18:45:16 by mhaziza          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	if_case_one(int is_pila, t_pset *pset)
{
	...
	push = is_pila ? PB : PA;
	swap = is_pila ? SA : SB;
	if ((!is_pila && pile->next->data > pile->next->next->data) || (is_pila && pile->next->data < pile->next->next->data))
	{
...
static void	if_case_two(int is_pila, t_pset *pset)
{
	...
	swap = is_pila ? SA : SB;
	rot = is_pila ? RA : RB;
	if ((!is_pila && pile->data > pile->next->data) || (is_pila && pile->data < pile->next->data))
		add_one_ope(swap, pset);
...
void		three_left_sort(int is_pila, t_pset *pset, int nsort)
{
	...
	if ((!is_pila && d1 > d2 && d1 > d3) || (is_pila && d1 < d2 && d1 < d3))
